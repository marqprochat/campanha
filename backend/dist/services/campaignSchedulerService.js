...
const wahaApiService_1 = require("./wahaApiService");
const evolutionMessageService_1 = require("./evolutionMessageService");
const quepasaMessageService_1 = require("./quepasaMessageService");
...
class CampaignSchedulerService {
    isRunning = false;
    intervalId = null;
    campaignSessionIndexes = new Map(); // Rastrear índice atual de cada campanha
    start() {
        if (this.isRunning) {
    ...
    async getNextSequentialSession(campaignId, sessionNames) {
        try {
    ...
    async startCampaign(campaign) {
        try {
            ...
            if (campaign.tenantId && websocketService_1.websocketService.isInitialized) {
                await websocketService_1.websocketService.notifyTenant(campaign.tenantId, {
    ...
    async processNextMessage(campaign, message) {
        ...
        try {
            ...
            let contactCheck = { exists: false };
            if (provider === 'EVOLUTION') {
                contactCheck = await (0, evolutionMessageService_1.checkContactExistsEvolution)(selectedSession, message.contactPhone);
            }
            else if (provider === 'QUEPASA') {
                contactCheck = await (0, quepasaMessageService_1.checkContactExistsQuepasa)(selectedSession, message.contactPhone, sessionToken);
            }
            else {
                contactCheck = await (0, wahaApiService_1.checkContactExists)(selectedSession, message.contactPhone);
            }
            ...
            let result;
            if (provider === 'EVOLUTION') {
                result = await this.sendMessageViaEvolution(selectedSession, contactCheck.validPhone || message.contactPhone, campaign.messageType, processedContent, contact, campaign.tenantId);
            }
            else if (provider === 'QUEPASA') {
                result = await this.sendMessageViaQuepasa(selectedSession, contactCheck.validPhone || message.contactPhone, campaign.messageType, processedContent, contact, campaign.tenantId, sessionToken);
            }
            else {
                result = await this.sendMessageViaWaha(selectedSession, message.contactPhone, campaign.messageType, processedContent, contactCheck.chatId, contact, campaign.tenantId);
            }
            if (result.success) {
    ...
    processVariables(content, contact) {
    ...
    selectRandomVariation(content) {
        ...
        if (content && typeof content === 'object' && (content.useVariations || content.useMediaVariations)) {
            ...
            if (content.mediaVariations && Array.isArray(content.mediaVariations) && content.mediaVariations.length > 0) {
                ...
                content.mediaVariations.forEach((variation, index) => {
                    console.log(`   Variation ${index + 1}: URL="${variation.url}", Caption="${variation.caption}"`);
                });
                // Filtrar apenas variações que têm URL válida
                const validVariations = content.mediaVariations.filter((variation) => variation.url && variation.url.trim() !== '');
        ...
        if (content && content.sequence && Array.isArray(content.sequence)) {
            const sequenceResults = content.sequence.map((item) => this.selectRandomVariation(item.content));
            const variationInfos = sequenceResults.map((result) => result.variationInfo).filter((info) => info !== null);
            return {
                processedContent: {
                    ...content,
                    sequence: content.sequence.map((item, index) => ({
                        ...item,
        ...
        if (Array.isArray(content)) {
            const arrayResults = content.map((item) => this.selectRandomVariation(item));
            const variationInfos = arrayResults.map((result) => result.variationInfo).filter((info) => info !== null);
            return {
                processedContent: arrayResults.map((result) => result.processedContent),
                variationInfo: variationInfos.length > 0 ? variationInfos.join('; ') : null
            };
        }
    ...
    async sendMessageViaEvolution(instanceName, phone, messageType, content, contactData, tenantId) {
        try {
            let result;
            switch (messageType) {
                case 'text':
                    result = await (0, evolutionMessageService_1.sendMessageViaEvolution)(instanceName, phone, { text: content.text });
                    break;
                case 'image':
                    result = await (0, evolutionMessageService_1.sendMessageViaEvolution)(instanceName, phone, {
                ...
                case 'openai':
                    ...
                    const openaiResult = await openaiService_1.openaiService.generateMessage(content, contactData, tenantId);
                    ...
                    result = await (0, evolutionMessageService_1.sendMessageViaEvolution)(instanceName, phone, { text: openaiResult.message });
                    break;
                case 'groq':
                    // Gerar mensagem usando Groq
                    console.log('⚡ Gerando mensagem com Groq (Evolution)...', content);
                    const groqResult = await groqService_1.groqService.generateMessage(content, contactData, tenantId);
                    if (!groqResult.success) {
                        throw new Error(`Groq error: ${groqResult.error}`);
                    }
                    console.log('✅ Mensagem gerada pela Groq (Evolution):', groqResult.message);
                    // Enviar a mensagem gerada como texto
                    result = await (0, evolutionMessageService_1.sendMessageViaEvolution)(instanceName, phone, { text: groqResult.message });
                    break;
                case 'sequence':
            ...
            return {
                success: true,
                messageId: result?.key?.id || result?.id || null
            };
        }
        catch (error) {
            return {
    ...
    async sendMessageViaQuepasa(sessionName, phone, messageType, content, contactData, tenantId, sessionToken) {
        try {
            let result;
            switch (messageType) {
                case 'text':
                    result = await (0, quepasaMessageService_1.sendMessageViaQuepasa)(sessionName, phone, { text: content.text }, sessionToken);
                    break;
                case 'image':
                    result = await (0, quepasaMessageService_1.sendMessageViaQuepasa)(sessionName, phone, {
                        image: { url: content.url },
                        caption: content.caption || ''
                    }, sessionToken);
                    break;
                ...
                case 'openai':
                    ...
                    // Enviar a mensagem gerada como texto
                    result = await (0, quepasaMessageService_1.sendMessageViaQuepasa)(sessionName, phone, { text: openaiResult.message }, sessionToken);
                    break;
                case 'groq':
                    // Gerar mensagem usando Groq
                    ...
                    // Enviar a mensagem gerada como texto
                    result = await (0, quepasaMessageService_1.sendMessageViaQuepasa)(sessionName, phone, { text: groqResult.message }, sessionToken);
                    break;
                case 'sequence':
                    // Para sequência, enviar todos os itens com delay entre eles
                    if (!content.sequence || content.sequence.length === 0) {
    ...
    async sendMessageViaWaha(sessionName, phone, messageType, content, validatedChatId, contactData, tenantId) {
        try {
            let result;
            switch (messageType) {
                case 'text':
                    result = await (0, wahaApiService_1.sendMessage)(sessionName, phone, { text: content.text }, validatedChatId);
                    break;
                case 'image':
                    result = await (0, wahaApiService_1.sendMessage)(sessionName, phone, {
                        image: { url: content.url },
                        caption: content.caption || ''
                    }, validatedChatId);
                    break;
                ...
                case 'openai':
                    ...
                    // Enviar a mensagem gerada como texto
                    result = await (0, wahaApiService_1.sendMessage)(sessionName, phone, { text: openaiResult.message }, validatedChatId);
                    break;
                case 'groq':
                    // Gerar mensagem usando Groq
                    ...
                    // Enviar a mensagem gerada como texto
                    result = await (0, wahaApiService_1.sendMessage)(sessionName, phone, { text: groqResult.message }, validatedChatId);
                    break;
                case 'sequence':
                    // Para sequência, enviar todos os itens com delay entre eles
                    if (!content.sequence || content.sequence.length === 0) {
    ...
    async completeCampaign(campaignId) {
        try {
            ...
            if (campaign && campaign.tenantId && websocketService_1.websocketService.isInitialized) {
                ...
                await automationService_1.automationService.executeTrigger(automationService_1.TriggerType.CAMPAIGN_COMPLETED, {
                    campaignId,
                    campaignName: campaign.nome,
