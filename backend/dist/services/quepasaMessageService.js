...
exports.checkContactExistsQuepasa = checkContactExistsQuepasa;
exports.sendMessageViaQuepasa = sendMessageViaQuepasa;
...
function detectMimeType(url) {
    ...
    if (urlLower.endsWith('.mp4'))
        return 'video/mp4';
    if (urlLower.endsWith('.avi'))
        return 'video/x-msvideo';
    if (urlLower.endsWith('.mov'))
        return 'video/quicktime';
    if (urlLower.endsWith('.wmv'))
        return 'video/x-ms-wmv';
    if (urlLower.endsWith('.flv'))
        return 'video/x-flv';
    if (urlLower.endsWith('.mkv'))
    ...
    if (urlLower.endsWith('.wav'))
        return 'audio/wav';
    if (urlLower.endsWith('.m4a'))
        return 'audio/mp4';
    if (urlLower.endsWith('.aac'))
        return 'audio/aac';
    if (urlLower.endsWith('.opus'))
        return 'audio/opus';
    // Documentos
 ...
 */
async function checkContactExistsQuepasa(sessionName, phone, sessionToken) {
    try {
 ...
 */
async function sendMessageViaQuepasa(sessionName, phone, content, sessionToken) {
    try {
        const quepasaConfig = await settingsService_1.settingsService.getQuepasaConfig();
        if (!quepasaConfig.url || !quepasaConfig.login) {
            throw new Error('Quepasa configuration is missing');
...
//# sourceMappingURL=quepasaMessageService.js.map