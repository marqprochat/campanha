    ...
    try {
        ...
        for (const tenant of tenants) {
            ...
            if (contactsUsage >= 90 && !await hasRecentAlert(tenant.id, 'QUOTA_WARNING', 'contacts')) {
                await (0, alertsController_1.createSystemAlert)('QUOTA_EXCEEDED', contactsUsage >= 100 ? 'CRITICAL' : 'HIGH', `Quota de contatos ${contactsUsage >= 100 ? 'excedida' : 'próxima do limite'}`, `Tenant ${tenant.name}: ${counts.contacts}/${quotas.maxContacts} contatos (${Math.round(contactsUsage)}%)`, {
            ...
            if (campaignsUsage >= 90 && !await hasRecentAlert(tenant.id, 'QUOTA_WARNING', 'campaigns')) {
                await (0, alertsController_1.createSystemAlert)('QUOTA_EXCEEDED', campaignsUsage >= 100 ? 'CRITICAL' : 'HIGH', `Quota de campanhas ${campaignsUsage >= 100 ? 'excedida' : 'próxima do limite'}`, `Tenant ${tenant.name}: ${counts.campaigns}/${quotas.maxCampaigns} campanhas (${Math.round(campaignsUsage)}%)`, {
                    tenantSlug: tenant.slug,
                    quotaType: 'campaigns',
            ...
            if (usersUsage >= 90 && !await hasRecentAlert(tenant.id, 'QUOTA_WARNING', 'users')) {
                await (0, alertsController_1.createSystemAlert)('QUOTA_EXCEEDED', usersUsage >= 100 ? 'CRITICAL' : 'HIGH', `Quota de usuários ${usersUsage >= 100 ? 'excedida' : 'próxima do limite'}`, `Tenant ${tenant.name}: ${counts.users}/${quotas.maxUsers} usuários (${Math.round(usersUsage)}%)`, {
                    tenantSlug: tenant.slug,
                    quotaType: 'users',
            ...
            if (sessionsUsage >= 90 && !await hasRecentAlert(tenant.id, 'QUOTA_WARNING', 'sessions')) {
                await (0, alertsController_1.createSystemAlert)('QUOTA_EXCEEDED', sessionsUsage >= 100 ? 'CRITICAL' : 'HIGH', `Quota de sessões ${sessionsUsage >= 100 ? 'excedida' : 'próxima do limite'}`, `Tenant ${tenant.name}: ${counts.whatsappSessions}/${quotas.maxConnections} sessões (${Math.round(sessionsUsage)}%)`, {
                    tenantSlug: tenant.slug,
                    quotaType: 'sessions',
        ...
        await (0, alertsController_1.createSystemAlert)('SYSTEM_ERROR', 'MEDIUM', 'Erro na verificação de quotas', `Falha ao executar verificação automática de quotas: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
    }
}
    ...
    try {
        ...
        for (const session of failedSessions) {
            if (!await hasRecentAlert(session.tenantId || undefined, 'SESSION_FAILED', session.id)) {
                await (0, alertsController_1.createSystemAlert)('SESSION_FAILED', 'HIGH', 'Sessão WhatsApp falhou', `Sessão "${session.name}" está com falha${session.tenant ? ` - Tenant: ${session.tenant.name}` : ''}`, {
        ...
        await (0, alertsController_1.createSystemAlert)('SYSTEM_ERROR', 'HIGH', 'Erro na verificação de saúde', `Falha ao executar verificação de saúde: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
    }
}
