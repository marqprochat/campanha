...
class AnalyticsService {
    ...
    async generateTenantAnalytics(tenantId, startDate, endDate) {
        try {
            ...
            const [totalContacts, totalCampaigns, activeUsers, contactsThisMonth, campaignsThisMonth, campaigns] = await Promise.all([
                // Total de contatos
    ...
    async generateSystemAnalytics(startDate, endDate) {
        try {
            const now = new Date();
            const start = startDate || new Date(now.getFullYear(), now.getMonth() - 11, 1); // Últimos 12 meses
            const end = endDate || now;
            ...
            const [totalSystemContacts, totalSystemCampaigns, totalSystemMessages] = await Promise.all([
                prisma.contact.count(),
                prisma.campaign.count(),
    ...
    async getCampaignPerformanceReport(tenantId, campaignId) {
        try {
    ...
    async exportTenantDataToCSV(tenantId, dataType) {
        try {
            switch (dataType) {
                case 'contacts':
                    ...
                    const contactsCsv = [
                        ...
                        ...contacts.map(c => `${c.id},"${c.nome}","${c.telefone}","${c.email || ''}","${c.tags?.join(';') || ''}","${c.criadoEm.toISOString()}"`)
                    ].join('\n');
                    return contactsCsv;
                case 'campaigns':
                    const campaigns = await prisma.campaign.findMany({
                    ...
                    const campaignsCsv = [
                        ...
                        ...campaigns.map(c => `${c.id},"${c.nome}","${c.status}","${c.criadoEm.toISOString()}","${c.scheduledFor?.toISOString() || ''}",${c.totalContacts || 0},${c.sentCount || 0},${c.failedCount || 0}`)
                    ].join('\n');
                    return campaignsCsv;
                case 'analytics':
