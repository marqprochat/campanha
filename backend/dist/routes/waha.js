...
const wahaSyncService_1 = require("../services/wahaSyncService");
...
router.get('/sessions', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        try {
            await wahaSyncService_1.WahaSyncService.syncAllSessions();
        }
        catch (wahaError) {
        ...
        try {
            ...
            const quepasaSessions = await whatsappSessionService_1.WhatsAppSessionService.getAllSessions(tenantId);
            const quepasaConfig = await settingsService_1.settingsService.getQuepasaConfig();
            ...
            if (quepasaConfig.url && quepasaConfig.login) {
                for (const session of quepasaSessionsFiltered) {
                    try {
                        ...
                        let sessionToken = session.quepasaToken;
                        ...
                        if (!sessionToken && session.status === 'WORKING') {
        ...
        try {
            ...
            const allEvolutionInstances = await evolutionApiService_1.evolutionApiService.listInstances();
            ...
            for (const instance of allEvolutionInstances) {
                try {
                    ...
                    try {
                        existingSession = await whatsappSessionService_1.WhatsAppSessionService.getSession(instanceName);
                    }
                    catch (error) {
                        // Sessão não existe no banco, será criada
                    }
                    // Obter status atualizado
                    const status = await evolutionApiService_1.evolutionApiService.getInstanceStatus(instanceName);
                    console.log(`🔍 Status Evolution para ${instanceName}:`, status);
                    // Obter informações detalhadas da instância
                    const instanceInfo = await evolutionApiService_1.evolutionApiService.getInstanceInfo(instanceName);
                    console.log(`📋 Info Evolution para ${instanceName}:`, JSON.stringify(instanceInfo, null, 2));
                    // Montar dados do 'me' quando conectado
                    let meData = undefined;
                    const evolutionData = instanceInfo;
        ...
        const updatedSessions = await whatsappSessionService_1.WhatsAppSessionService.getAllSessions(tenantId);
...
router.get('/sessions/:sessionName', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        try {
            const session = await whatsappSessionService_1.WhatsAppSessionService.getSession(sessionName, tenantId);
        ...
        const session = await wahaSyncService_1.WahaSyncService.syncSession(sessionName);
        res.json(session);
    }
    catch (error) {
...
// Criar nova sessão
router.post('/sessions', auth_1.authMiddleware, quotaMiddleware_1.checkConnectionQuota, async (req, res) => {
    try {
        ...
        if (provider === 'EVOLUTION') {
            const { evolutionApiService } = await Promise.resolve().then(() => __importStar(require('../services/evolutionApiService')));
            ...
            result = await wahaSyncService_1.WahaSyncService.createSession(realName);
...
// Iniciar sessão
router.post('/sessions/:sessionName/start', auth_1.authMiddleware, async (req, res) => {
    try {
        const { sessionName } = req.params;
        ...
        let sessionData;
        try {
            sessionData = await whatsappSessionService_1.WhatsAppSessionService.getSession(sessionName, tenantId);
        ...
        if (sessionProvider === 'EVOLUTION') {
            ...
            try {
                ...
                result = await evolutionApiService_1.evolutionApiService.getQRCode(sessionName);
...
// Parar sessão
router.post('/sessions/:sessionName/stop', auth_1.authMiddleware, async (req, res) => {
    try {
        const { sessionName } = req.params;
        ...
        if (sessionProvider === 'EVOLUTION') {
            ...
            result = await wahaSyncService_1.WahaSyncService.stopSession(sessionName);
        }
        res.json(result);
    }
    catch (error) {
...
router.post('/sessions/:sessionName/restart', async (req, res) => {
    try {
        ...
        let sessionProvider = 'WAHA';
        try {
            const savedSession = await whatsappSessionService_1.WhatsAppSessionService.getSession(sessionName);
        ...
        let result;
        if (sessionProvider === 'EVOLUTION') {
            result = await evolutionApiService_1.evolutionApiService.restartInstance(sessionName);
            await whatsappSessionService_1.WhatsAppSessionService.createOrUpdateSession({
                name: sessionName,
                status: 'SCAN_QR_CODE',
                provider: 'EVOLUTION'
            ...
            result = await wahaSyncService_1.WahaSyncService.restartSession(sessionName);
        }
        res.json(result);
    }
    catch (error) {
...
// Deletar sessão
router.delete('/sessions/:sessionName', auth_1.authMiddleware, async (req, res) => {
    try {
        const { sessionName } = req.params;
        ...
        let sessionProvider = 'WAHA';
        try {
            const savedSession = await whatsappSessionService_1.WhatsAppSessionService.getSession(sessionName, tenantId);
            sessionProvider = savedSession.provider || 'WAHA';
        }
        catch (error) {
            console.error('❌ Sessão não encontrada ou não pertence ao tenant:', error);
        ...
        if (sessionProvider === 'EVOLUTION') {
            try {
                await evolutionApiService_1.evolutionApiService.deleteInstance(sessionName);
            ...
            try {
                await whatsappSessionService_1.WhatsAppSessionService.deleteSession(sessionName, tenantId);
            ...
            try {
                ...
                if (quepasaConfig.url && quepasaConfig.login) {
                    ...
                    let sessionToken = savedSession.quepasaToken;
            ...
            await wahaSyncService_1.WahaSyncService.deleteSession(sessionName);
        }
...
router.get('/sessions/:sessionName/auth/qr', auth_1.authMiddleware, async (req, res) => {
    try {
        const { sessionName } = req.params;
        ...
        if (sessionProvider === 'EVOLUTION') {
            try {
                const qrCodeData = await evolutionApiService_1.evolutionApiService.getQRCode(sessionName);
...
router.get('/sessions/:sessionName/status', auth_1.authMiddleware, async (req, res) => {
    try {
        const { sessionName } = req.params;
        ...
        try {
            await whatsappSessionService_1.WhatsAppSessionService.getSession(sessionName, tenantId);
        }
        catch (error) {
            console.error('❌ Sessão não encontrada ou não pertence ao tenant:', error);
            return res.status(404).json({ error: 'Sessão não encontrada' });
...
router.get('/sessions/:sessionName/me', auth_1.authMiddleware, async (req, res) => {
    try {
        const { sessionName } = req.params;
...
router.patch('/sessions/:sessionName/assign-tenant', auth_1.authMiddleware, async (req, res) => {
    try {
        const { sessionName } = req.params;
        ...
        const session = await whatsappSessionService_1.WhatsAppSessionService.getSession(sessionName);
