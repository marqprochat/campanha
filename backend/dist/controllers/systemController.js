...
exports.getQuotasAlerts = exports.refreshStats = exports.getSystemHealth = exports.getTenantStats = exports.getSystemStats = void 0;
...
const getSystemStats = async (req, res) => {
    try {
        ...
        const [tenantsStats, usersStats, contactsCount, campaignsStats, sessionsStats, messagesCount, activeToday, alertsStats] = await Promise.all([
        ...
        usersStats.forEach(stat => {
            ...
            if (stat.role in userData.byRole) {
                userData.byRole[stat.role] = stat._count;
            }
        });
...
const getQuotasAlerts = async (req, res) => {
    try {
        // Verificar se é SUPERADMIN ou ADMIN (temporário)
        if (req.user?.role !== 'SUPERADMIN' && req.user?.role !== 'ADMIN') {
            return res.status(403).json({ error: 'Acesso negado. Apenas SUPERADMINs podem acessar.' });
...
exports.getQuotasAlerts = getQuotasAlerts;
