...
exports.downloadCampaignReport = exports.getActiveSessions = exports.getContactTags = exports.getCampaignReport = exports.toggleCampaign = exports.deleteCampaign = exports.updateCampaign = exports.createCampaign = exports.getCampaign = exports.listCampaigns = exports.campaignValidation = void 0;
...
exports.campaignValidation = [
    (0, express_validator_1.body)('nome').notEmpty().withMessage('Nome da campanha é obrigatório'),
    (0, express_validator_1.body)('targetTags').isArray().withMessage('Categorias dos contatos devem ser um array'),
    (0, express_validator_1.body)('sessionNames').isArray({ min: 1 }).withMessage('Pelo menos uma sessão WhatsApp deve ser selecionada'),
    (0, express_validator_1.body)('messageType').isIn(['text', 'image', 'video', 'audio', 'document', 'sequence', 'openai', 'groq', 'wait']).withMessage('Tipo de mensagem inválido'),
...
const createCampaign = async (req, res) => {
    try {
        ...
        const campaignMessages = filteredContacts.map((contact) => ({
...
const getCampaignReport = async (req, res) => {
    try {
        ...
        const sessionNames = [...new Set(campaign.messages.map(m => m.sessionName).filter(Boolean))];
        let sessionsInfo = [];
        let sessionProviderMap = {};
        if (sessionNames.length > 0) {
            try {
                ...
                sessionProviderMap = sessionsInfo.reduce((acc, session) => {
                    acc[session.name] = {
        ...
        const messagesBySession = messagesWithProvider.reduce((acc, message) => {
...
const getContactTags = async (req, res) => {
    try {
        ...
        const tenantId = req.tenantId;
        const categoriesResponse = await categoryService_1.CategoryService.getCategories(undefined, 1, 999999, tenantId);
        // Retornar array com id e nome das categorias
        const tags = categoriesResponse.categories.map((categoria) => ({
            id: categoria.id,
            nome: categoria.nome
        }));
        res.json(tags);
...
exports.downloadCampaignReport = downloadCampaignReport;
